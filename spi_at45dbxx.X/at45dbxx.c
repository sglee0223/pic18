...
int at45db_rdid(void)
{
    ...
    while(!spi_data_ready());   // wait for a data to arrive
    
	spi_read(devid, sizeof(devid));
...
void at32db_chiperase(void)
{
    ...
    spi_write(g_chiperase, CHIP_ERASE_SIZE);
...
void at45db_pgwrite(uint8_t *buffer, long page)
{
    ...
    spi_read(buffer, 1 << priv.pageshift);
...
int at45db_initialize(void)
{
    ...
    if ((sr & AT45DB_SR_PGSIZE) == 0)
    {
        ...
        spi_write(g_binpgsize, BINPGSIZE_SIZE);
