...
#define SELECT()	LATAbits.LA5 = 0		/* CS = L */
#define	DESELECT()	LATAbits.LA5 = 1		/* CS = H */
#define	MMC_SEL		!PORTAbits.RA5		/* CS status (true:CS == L) */
#define	FORWARD(d)	printf("%c", d) //uart_put(d)		/* Data forwarding function (Console out in this example) */

void init_spi (void)		/* Initialize SPI port */
{
    SSPCON1 = 0x31; // SSPCON1: MSSP CONTROL REGISTER 1 (SPI MODE)
